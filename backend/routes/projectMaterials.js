...
router.get('/:projectId', (req, res) => {
  ...
  const query = 'SELECT * FROM budgetItems WHERE projectId = ?';
...
router.post('/', (req, res) => {
  ...
  const query = 'INSERT INTO budgetItems (projectId, name, value) VALUES (?, ?, ?)';
...
router.put('/:id', (req, res) => {
  ...
  const query = 'UPDATE budgetItems SET name = ?, value = ? WHERE id = ?';
...
router.delete('/:id', (req, res) => {
  const query = 'DELETE FROM budgetItems WHERE id = ?';
